...
async function batchInsertJobs(db, records, clearFirst = true, fy = null) {
  ...
  for (const [tabName, rows] of Object.entries(records)) {
    ...
    try {
      ...
      // clearFirst only on first chunk — subsequent chunks append
      if (clearFirst) {
...
async function updateUserFields(db, email, fields) {
  ...
  if (fields.region    !== undefined) allowed.region    = String(fields.region || "").trim();
...
// ── RLS: resolve which reps a user is allowed to see ─────────────────────────
// Ocean hierarchy: Sales Rep → Zonal Manager → Regional Manager (N/W/E/S) → Admin/HR
// Regional Manager sees all reps whose zone maps to their region.
// zone→region mapping comes from mapping_zone_targets "Region" column.
async function _getRLSReps(db, currentUser) {
  const role   = String(currentUser.role   || "").toLowerCase().trim();
  const zone   = String(currentUser.zone   || "").trim();
  const region = String(currentUser.region || "").trim();
  ...
  const zoneRegionMap = {};
  for (const r of zoneTargetRows) {
    const z = String(r["Zone"]||"").trim(), reg = String(r["Region"]||"").trim();
    if (z && reg) zoneRegionMap[z] = reg;
  }
  ...
  if (role === "regional manager") {
    ...
    for (const row of mappingRows) {
      if (allowedZones.has(String(row["Zone"]||"").trim())) {
        const norm = normalizeName(row["Sales Rep Name"]);
        const dn   = normalizeName(row["Display Name"] || row["Sales Rep Name"]);
        if (norm) allowedReps.add(norm);
        if (dn)   allowedReps.add(dn);
      }
    }
    return allowedReps;
  }
  ...
  // Sales Rep — self only
  const selfNorm = normalizeName(currentUser.name || "");
  const selfSet = new Set();
  if (selfNorm) selfSet.add(selfNorm);
  for (const row of mappingRows) {
    const repEmail = String(row["Email ID"]||"").toLowerCase().trim();
    if (repEmail === email) {
      selfSet.add(normalizeName(row["Sales Rep Name"]));
      selfSet.add(normalizeName(row["Display Name"] || row["Sales Rep Name"]));
    }
  }
  return selfSet;
}

const DEPLOY_TS = "2026-08-21T-ocean-v195-teu-debug-fix-1787632267";
let salesCache = null;
...
async function computeSalesAggregate(db) {
    ...
    for (const job of jobs) {
      ...
      if (!mapped) {
        ...
        // LOB accumulation (Sea Export/Import, ISOTANK Export/Import, Air Export/Import)
        {
          ...
          branchLobData[branch][lobKey][monthLabel].rev       += rev;
          branchLobData[branch][lobKey][monthLabel].revBilled += revBilledAmt;
          branchLobData[branch][lobKey][monthLabel].revProv   += revProvAmt;
        }
        continue;
      }
      ...
      // Use FY key matching yearGroups format: FY25=FY2025-26, FY26=FY2026-27
      const _jobFY = ['Apr-26','May-26','Jun-26','Jul-26','Aug-26','Sep-26','Oct-26','Nov-26','Dec-26','Jan-27','Feb-27','Mar-27'].includes(monthLabel) ? 'FY26' : 'FY25';
  ...
  for (const rep of repsRaw) {
    if (!zonesMap[rep.zone]) {
      ...
      // Use FY26 target only when ALL active months are in FY26 (Apr-25 to Mar-26)
      const fy26MonthSet = new Set(["Apr-25","May-25","Jun-25","Jul-25","Aug-25","Sep-25","Oct-25","Nov-25","Dec-25","Jan-26","Feb-26","Mar-26"]);
      const isEntirelyFY26 = activeMonths.length > 0 && activeMonths.every(m => fy26MonthSet.has(m));
      const chosenTarget = isEntirelyFY26 ? (fy26Target || fy27Target || {}) : (fy27Target || fy26Target || {});
      zonesMap[rep.zone] = {
        zone: rep.zone,
  ...
  await Promise.all(ALL_COLLS.map(async (collName) => {
    ...
    for (const job of jobs) {
      ...
      if (!isAir) {
        ...
        if (_localTeuDebug.length < 8 && parseFloat(job["Container TEU"]||0) > 0 && _cargoTypeCI === "LCL") {
          _localTeuDebug.push({sno: job["Shipment No"], rawCargoType: job["Cargo Type"], cargoTypeCI: _cargoTypeCI, teuAssigned: teu, containerTeuRaw: job["Container TEU"], coll: collName});
        }
      }
...
async function computeTradelaneAggregate(db) {
  ...
  await Promise.all(JOB_COLL_MAP.map(async (cfg) => {
    ...
    for (const job of jobs) {
      ...
      if (srrEntry) {
        // SRR match — tradelane only, empty → Others
...
async function computeBothPendency(db) {
  const mappingRows = await db.collection("ocean_mapping_sales_targets").find(
...
module.exports = async function handler(req, res) {
  ...
  if (action === "ping") {
    ...
    if (salesResult && salesResult.repsRaw) {
      salesStripped = { ...salesResult, repsRaw: salesResult.repsRaw.map(r => {
        ...
        if (c.lobData) {
          // Keep ALL LOBs in lobData — needed for LOB filter to show correct GP/Revenue per LOB
          // Previously stripped to Air-only but that caused Sea/Isotank to show 0 when LOB filter active
        }
        return c;
      })};
    ...
    if (action === "sales") {
      ...
      const strip = (result) => ({ ...result, repsRaw: result.repsRaw.map(r => {
        ...
        if (includeLob) {
          ...
          // Always include Ocean primary LOB data (SEA + ISOTANK) for default filter
          if (copy.lobData) {
            const oceanOnly = {};
            if (copy.lobData['SEA EXPORT'])     oceanOnly['SEA EXPORT']     = copy.lobData['SEA EXPORT'];
  ...
  if (action === "addUser") {
    ...
    if (action === "srr") {
      ...
      const ALLOWED_SRR = new Set(["srr_sea_export","srr_sea_import","srr_air_export","srr_air_import","srr_isotank_export","srr_isotank_import"]);
    ...
    if (action === "wipeSRRByFY") {
      ...
      const ALLOWED_SRR = ["srr_sea_export","srr_sea_import","srr_air_export","srr_air_import","srr_isotank_export","srr_isotank_import"];
  ...
  for (const cfg of cfgs) {
    ...
    for (const job of jobs) {
      ...
      const { gp } = pickGP(job, cls);
      const _isLocked = !!(String(job["Job Rev Recognition Date"] || "").trim());
      const provRev = parseFloat(job["Provisional Revenue (A)"] || 0) || 0;
      const billRev = parseFloat(job["Billed Revenue (C)"] || 0) || 0;
      const rev = _isLocked ? billRev : provRev;
      const provCost = parseFloat(job["Provisional Cost (E)"] || 0) || 0;
      ...
      rows.push({
        ...
        consolNo: job["Consol No."]||"", cargoType: job["Cargo Type"]||"",
        carrier: job["Carrier"]||"", customer: job["Customer"]||"",
        consignee: job["Consignee"]||"", shipper: job["Shipper"]||"",
        consolType: job["Consol Type"]||"",
        teu: parseFloat(job["Container TEU"]||0)||0,
        destAgent: job["Destination Agent"]||"", originAgent: job["Origin Agent"]||"",
        etaDischarge: job["ETA Discharge"]||"", etdLoading: job["ETD Loading Port"]||"",
        ataDischarge: job["ATA Discharge"]||"", atdLoading: job["ATD Loading Port"]||"",
        jobRevRecogDate: job["Job Rev Recognition Date"]||"",
        provRevenue: provRev, billedRevenue: billRev, unbilledRevenue: provRev-billRev,
        provCost, postedCost: postCost, unpostedCost: provCost-postCost,
        provisionalProfit: provRev-provCost, actualProfit: billRev-postCost,
        g: gp, r: rev, gp, revenue: rev,
        salesPerson: job["Sales Person"]||"", jobOwner: job["Job Owner"]||"",
        location: job["Location"]||"", operationLock: job["Operation Lock"]||"",
        financialLock: job["Financial Lock"]||"",
        volume: parseFloat(job["Volume"]||0)||0, volumeUnit: job["Volume Unit"]||"",
